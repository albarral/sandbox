...
float Learn::maxQFromState(State& oFromState)
{
    float maxQ = 0.0;
    float Q;
    std::vector<Transition>& listTransition = oFromState.getListTransitions();
    
    std::vector<Transition>::iterator it_transition = listTransition.begin();
    std::vector<Transition>::iterator it_end = listTransition.end();
